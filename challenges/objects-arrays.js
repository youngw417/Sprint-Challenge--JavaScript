...
const universites = graduates.map(cur => cur.university);


Once you have the new array created, sort the universities alphabetically and log the result. */
//
const universities = graduates.map(cur => cur.university);
...
const contactInfo = graduates.map(cur => `${cur.first_name} ${cur.email}`);
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
let unisWithUni = [];
graduates.forEach(cur =>{
  if (cur.university.includes('Uni')) unisWithUni.push(cur);
})
...
*/
const displayNames = [];
zooAnimals.forEach(cur => displayNames.push(`Name: ${cur.animal_name}, Scientific: ${cur.scientific_name}`));
console.log(displayNames);

/* Request 2: .map()

The zoos need a list of all their animal's names (animal_name only) converted to lower case. Using map, 
create a new array of strings named lowCaseAnimalNames, each string following 
this pattern: "jackal, asiatic". Log the resut.

*/

let lowCaseAnimalNames = [];
lowCaseAnimalNames = zooAnimals.map(cur => cur.animal_name.toLowerCase());
...
lowPopulationAnimals = zooAnimals.filter(cur => cur.population < 5)
